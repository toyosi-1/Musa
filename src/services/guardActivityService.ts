...
export const logVerificationAttempt = async (
  ...
  try {
    ...
    // Log to unified activity feed so all dashboards see it
    // We need the estateId — fetch it from the access code if available
    if (verificationData.accessCodeId || verificationData.householdId) {
      try {
        let estateId = '';
        if (verificationData.accessCodeId) {
          const codeSnap = await get(ref(db, `accessCodes/${verificationData.accessCodeId}`));
          if (codeSnap.exists()) estateId = codeSnap.val()?.estateId || '';
        }
        if (estateId) {
          await logActivity({
            type: verificationData.isValid ? 'guest_checkin' : 'guest_denied',
            description: verificationData.isValid
              ? `Guest checked in${verificationData.destinationAddress ? ` — ${verificationData.destinationAddress.split('\n')[0]}` : ''}`
              : `Entry denied: ${verificationData.message || 'Invalid code'}`,
            timestamp,
            userId: guardId,
            estateId,
            householdId: verificationData.householdId,
            metadata: {
              guardId,
        ...
        console.error('[guardActivityService] logActivity failed (non-fatal):', actErr);
      }
    }
