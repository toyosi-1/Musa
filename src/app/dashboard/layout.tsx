      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm shadow-lg border-t border-gray-100 dark:border-gray-700 pb-[max(env(safe-area-inset-bottom),0.5rem)] z-50 transform-gpu will-change-transform">
        <div className="flex justify-around items-stretch max-w-md mx-auto px-2 py-1.5 sm:py-2">
            ...
            <div className="relative flex flex-col items-center justify-center h-full">
              ...
              >
                <div className="relative -mt-5">
                  <div className="absolute -top-5 left-1/2 transform -translate-x-1/2 w-14 h-14 bg-musa-red hover:bg-red-600 rounded-full flex items-center justify-center shadow-lg transition-all active:scale-95">
          ...
          <div className="relative group h-full">
            <div className="flex flex-col items-center p-2 rounded-lg min-w-[60px] h-full justify-center">
              <div className="w-8 h-8 rounded-full bg-musa-blue/10 dark:bg-blue-900/30 flex items-center justify-center text-musa-blue dark:text-blue-400 group-hover:bg-musa-blue/20 dark:group-hover:bg-blue-900/50 transition-colors">
                {currentUser.displayName?.charAt(0).toUpperCase() || 'U'}
              </div>
              <span className="text-xs font-medium text-gray-500 dark:text-gray-400 group-hover:text-musa-blue dark:group-hover:text-blue-400 transition-colors truncate max-w-[60px] mt-0.5">
                {currentUser.displayName?.split(' ')[0] || 'User'}
              </span>
            </div>
            <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 w-48 bg-white dark:bg-gray-800 rounded-lg shadow-xl py-1 z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 border border-gray-100 dark:border-gray-700">
              <button 
                onClick={handleSignOut} 
                className="w-full text-left px-4 py-2.5 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/80 flex items-center transition-colors active:bg-gray-100 dark:active:bg-gray-600"
                aria-label="Sign out"
              >
                <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4 mr-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
