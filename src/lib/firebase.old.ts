...
export type FirebaseAppType = FirebaseApp;
...
export async function getFirebaseDatabase(): Promise<Database> {
  ...
  if (!firebaseDb) {
    ...
    try {
      ...
      // Set shorter timeouts and better connection handling
      try {
        // Set database operation timeout to 15 seconds (default is 60s)
        (firebaseDb as any).app.options.databaseTimeoutSeconds = 15;
        
        // Enable persistence for better offline support
        await import('firebase/database');
        
        console.log('⏱️ Configured database with 15s timeout');
      } catch (e) {
        console.warn('⚠️ Could not configure database settings:', e);
      }
